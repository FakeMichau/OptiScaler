        ...
        if (dllPath.filename().wstring() == L"libxess.dll")
        {
            if (Config::Instance()->XeSSLibrary.has_value())
            {
                ...
                if (cfgPath.has_filename())
                {
                    ...
                    Util::GetDLLVersion(cfgPath.wstring(), reinterpret_cast<Util::version_t*>(&_xessVersion));
            ...
            if (_dll != nullptr)
            {
                ...
                Util::GetDLLVersion(path.wstring(), reinterpret_cast<Util::version_t*>(&_xessVersion));
            }
            else
            {
