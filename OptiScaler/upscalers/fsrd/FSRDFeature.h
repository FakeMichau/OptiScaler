...
inline static void FfxdLogCallback(uint32_t type, const wchar_t* message)
{
    std::wstring string(message);
    LOG_DEBUG("FSR Runtime: {0}", wstring_to_string(string));
}

class FSRDFeature : public virtual IFeature
{
  private:
  ...
  protected:
    ...
    ffxCreateContextDescUpscale _upscaleContextDesc = {};
    ffxCreateContextDescDenoiser _denoiserContextDesc = {};

    virtual bool InitFSRD(const NVSDK_NGX_Parameter* InParameters) = 0;
  ...
  public:
    ...
    FSRDFeature(unsigned int InHandleId, NVSDK_NGX_Parameter* InParameters);
