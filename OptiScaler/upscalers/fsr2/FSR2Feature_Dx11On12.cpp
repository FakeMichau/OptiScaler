...
bool FSR2FeatureDx11on12::Evaluate(ID3D11DeviceContext* InDeviceContext, NVSDK_NGX_Parameter* InParameters)
{
    ...
    if (!_baseInit)
    {
        ...
        if (!Config::Instance()->DisplayResolution.value_or(false) && !State::Instance().DisplaySizeMV.value_or(false))
        {
            ...
            if (!State::Instance().DisplaySizeMV.has_value() && paramVelocity != nullptr)
            {
...
#pragma endregion
    ...
    if (useSS)
    {
        ...
        if (!OutputScaler->Dispatch(Dx12Device, Dx12CommandList, OutputScaler->Buffer(), dx11Out.Dx12Resource, Handle()->Id))
        {
            Config::Instance()->OutputScalingEnabled.set_volatile_value(false);
